...
feature '職員による顧客管理' do
  ...
  scenario '職員が生年月日と自宅の住所に無効な値を入力する' do
    click_link '顧客管理'
    first('table.listing').click_link '編集'
  ...
  scenario '職員が勤務先のない既存顧客に勤務先を追加する' do
    ...
    within('fieldset#work-address-fields') do
      fill_in '会社名', with: 'テストテスト'
    end
    click_button '更新'

    customer.reload
    expect(customer.work_address.company_name).to eq('テストテスト')
  end
