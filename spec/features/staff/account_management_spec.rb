...
feature '職員による自分自身のアカウント管理' do
  ...
  scenario '職員がメールアドレス、氏名、ふりがなを更新する' do
    ...
    fill_in 'staff_member_given_name', with: 'ハナコ'
    click_button '確認画面へ進む'
    click_button '訂正'
    fill_in 'staff_member_family_name', with: 'シケン'
    fill_in 'staff_member_given_name', with: 'ハナコ'
    click_button '確認画面へ進む'
    click_button '更新'

    staff_member.reload
    expect(staff_member.family_name).to eq('シケン')
    expect(staff_member.given_name).to eq('ハナコ')
  end
