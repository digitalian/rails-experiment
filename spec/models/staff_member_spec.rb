...
describe StaffMember do
  describe '#password=' do
    example '文字列を与えると、hashed_keywordは長さ60の文字列になる' do
    ...
    example 'nilを与えると、hashed_keywordはnilになる' do
  ...
  describe '値の正規化' do
    ...
    example 'email前後の全角スペースを除去' do
      member = create(:staff_member, email: "\u{3000}ｔｅｓｔ＠ｅｘａｍｐｌｅ．ｃｏｍ\u{3000}")
      expect(member.email).to eq('test@example.com')
    end
  ...
  describe 'バリデーション' do
    ...
    example 'スペースを含むfamily_nameは無効' do
      member = build(:staff_member, family_name: "\u{3000}試験")
      expect(member).to be_valid
    end
